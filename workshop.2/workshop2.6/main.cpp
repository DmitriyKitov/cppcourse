...
void pollEvents(sf::RenderWindow &window, sf::Vector2f &pos)
{
    sf::Event event;
...
{
    Ball ball = {40, color, pos, speed, 10};
...
bool ball_near(const std::vector<Ball> &balls, const sf::Vector2f &pos)
{
    for (int i = 0; i < balls.size(); i++)
    {
        if (dist(pos, balls[i].pos) < 40 + balls[i].radius)
        {
            return true;
        }
    }
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        auto removeDeadBalls = [&](Ball ball) -> bool {
            return ball.life < 0;
        };
        auto iterator = std::remove_if(balls.begin(), balls.end(), removeDeadBalls);
        balls.erase(iterator, balls.end());

        if (pos.x != 0 || pos.y != 0)
        {
            if (!ball_near(balls, pos))
            {
                addBall(balls, rnd_color2(generator, pal), pos, rnd_speed(generator));
            }
        }
